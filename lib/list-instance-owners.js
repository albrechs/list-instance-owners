...
const findTag = (tags, searchTag) => {
  let tagValue
  let tagQuery = query('Key').is(searchTag).on(tags)
...
const parseInstances = (reservations, tagQuery) => {
  ...
  return sortObject(output, tagQuery)
}

const sortObject = (obj, tagQuery) => {
  const tagSort = (a, b) => {
    let comparison = 0
    if (a[tagQuery].toLowerCase() > b[tagQuery].toLowerCase()) {
      comparison = 1
    } else if (a[tagQuery].toLowerCase() < b[tagQuery].toLowerCase()) {
      comparison = -1
    }
    return comparison
  }
  return obj.sort(tagSort)
}
